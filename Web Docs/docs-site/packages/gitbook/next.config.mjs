...
const vercelDeploymentId = process.env.NEXT_DEPLOYMENT_ID;
let deploymentId =
    process.env.GITBOOK_RUNTIME === 'cloudflare'
        ...
        : vercelDeploymentId ||
          ...
          Date.now().toString(); // Needed because we use a custom deployment method i.e. https://vercel.com/docs/skew-protection#custom-deployment-id

const { VERCEL_TARGET_ENV } = process.env;

// Because preview, staging and prod share the same SHA, prefix environment when generating our own id.
